...
about the same time. The `async_comprehension` func calls the
`async_generator` func which waits 1 second, then yields a random num.
...
async def measure_runtime() -> float:
    ...
    This func starts a timer, then concurrently executes four instances of
    the `async_comprehension` func using asyncio.gather. It calculates the
